...
const AutoHint = ({ title, description }) => {
  const [open, setOpen] = React.useState(true);
  const handleClose = () => setOpen(false);

  return <Snackbar sx={{ height: '100%' }} anchorOrigin={{ vertical: "top", horizontal: "center" }}
...
const setLocalStorage = async (key, value) => {
  const tabStorage = await getTabStorage();
  const newStorage = { ...tabStorage, [key]: value };
  return new Promise((resolve) => {
    chrome.storage.local.set({ [CURRENT_TAB_ID]: newStorage }, (result) => {
      resolve(result);
    });
  });
};
...
const DriverLogin = () => {
  ...
  React.useEffect(async () => {
    ...
    const clickSignin = () => {
      signInButton().click();
      // if entries are good, window.location will navigate to below
    ...
    if (value) {
      ...
      const { lastName, licenseNumber, keyWord } = value;
  ...
  return <AutoHint {...signInHint} />;
}

const findRaisedButtonByText = (text) => {
  return Array.from(document.querySelectorAll('button.mat-raised-button'))
    .filter(n => n.innerText.includes(text))[0];
}

const findStrokeButtonByText = (text) => {
  return Array.from(document.querySelectorAll('button.mat-stroked-button'))
    .filter(n => n.innerText.includes(text))[0];
}
  ...
  React.useEffect(() => {
    ...
    let interval = setInterval(() => {
      ...
      if (adt) {
        ...
        findRaisedButtonByText('Yes').click();
        // will navigate to https://onlinebusiness.icbc.com/webdeas-ui/booking
      }
    }, 100);
    return () => {
...
const icbcOfficeDiv = () => document
  .querySelectorAll('div.appointment-location-wrapper')[Math.round(Math.random())];
    ...
    let searchTimer = setInterval(async () => {
      try {
        ...
        let dateAfter30Days = () => {
          ...
          earliestDate = earliestDate.substring(
            earliestDate.indexOf(',') + 1).trim()
        ...
        console.log('Chose the earilest time slot on',
          firstAvalableDate().innerText, 'at', tb().textContent.trim());

        // choose the earilest time slot and click 'Review Appointment'
        ...
        const cancelButton = () =>
          Array.from(document.querySelectorAll('button.mat-stroked-button'))
            .filter(n => n.innerText.includes('Cancel'))[1]
...
const BookRoadTest = () => {
  ...
  React.useEffect(async () => {
    ...
    const searchLocationCallback = () => {
      ...
      setLocalStorage(LOCATIONDETAILS_KEY, { locations });
  ...
  return !locationDetails && <AutoHint {...searchLocationHint} />;
};
...
chrome.runtime.sendMessage({ type: "CONTENT_HELLO" }, (res) => {
...
chrome.runtime.onMessage.addListener(
  function (request, sender, sendResponse) {
    ...
    if (request.type === 'TAB_UPDATED' &&
      (!CURRENT_TAB_ID || request.tab.id.toString() === CURRENT_TAB_ID) &&
      request.changeInfo.status === 'complete') {
      renderApp();
    }
  }
);
